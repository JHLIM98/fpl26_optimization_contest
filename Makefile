...
run_strategy:
	...
	echo ""; \
	$(PYTHON) dcp_optimizer.py "$(DCP)" $(if $(OUTPUT),--output "$(OUTPUT)") $(if $(RUN_DIR),--run-dir "$(RUN_DIR)") --strategy $(STRATEGY) $(if $(PBLOCK_RANGES),--pblock-ranges "$(PBLOCK_RANGES)") $(if $(PBLOCK_UTIL_FACTOR),--pblock-util-factor $(PBLOCK_UTIL_FACTOR))
